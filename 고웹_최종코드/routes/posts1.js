...
var Post = require('../models/Post1');
var User = require('../models/User');
var Comment = require('../models/Comment1');
  ...
  if(searchQuery) {
    ...
    posts = await Post.aggregate([
      ...
      { $project: {
          ...
          createdAt: 1,
          commentCount: { $size: '$comments1'}
      } },
    ]).exec();
  }

  res.render('posts1/index', {
    posts:posts,
    currentPage:page,
    maxPage:maxPage,
    limit:limit,
...
router.get('/new', util.isLoggedin, function(req, res){
  ...
  res.render('posts1/new', { post:post, errors:errors });
});
...
router.post('/', util.isLoggedin, upload.single('attachment'), async function(req, res){
  ...
  Post.create(req.body, function(err, post){
    if(err){
      ...
      return res.redirect('/posts1/new'+res.locals.getPostQueryString());
    }
    if(attachment){
      attachment.postId = post._id;
      attachment.save();
    }
    res.redirect('/posts1'+res.locals.getPostQueryString(false, { page:1, searchText:'' }));
  });
});
...
router.get('/:id', function(req, res){
  ...
  Promise.all([
    ...
    .then(([post, comments1]) => {
      post.views++;
      post.save();
      var commentTrees = util.convertToTrees(comments1, '_id','parentComment','childComments');
      res.render('posts1/show', { post:post, commentTrees:commentTrees, commentForm:commentForm, commentError:commentError});
    })
    .catch((err) => {
      return res.json(err);
    });
  ...
  if(!post){
    Post.findOne({_id:req.params.id})
      ...
      .exec(function(err, post){
        ...
        res.render('posts1/edit', { post:post, errors:errors });
      });
  }
  else {
...
router.put('/:id', util.isLoggedin, checkPermission, upload.single('newAttachment'), async function(req, res){
  ...
  Post.findOneAndUpdate({_id:req.params.id}, req.body, {runValidators:true}, function(err, post){
    if(err){
      req.flash('post', req.body);
      req.flash('errors', util.parseError(err));
      return res.redirect('/posts1/'+req.params.id+'/edit'+res.locals.getPostQueryString());
    }
    res.redirect('/posts1/'+req.params.id+res.locals.getPostQueryString());
  });
});
...
router.delete('/:id', util.isLoggedin, checkPermission, function(req, res){
  Post.deleteOne({_id:req.params.id}, function(err){
    if(err) return res.json(err);
    res.redirect('/posts1'+res.locals.getPostQueryString());
  });
});
